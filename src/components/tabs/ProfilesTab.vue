...
const props = defineProps({
  ...
  paginatedProfiles: { type: Array, required: true },
  profilesCurrentPage: { type: Number, required: true },
  profilesTotalPages: { type: Number, required: true },
...
<template>
  <div class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-4 sm:p-8 lg:p-10 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        ...
        <div class="relative" ref="profilesMoreMenuRef">
          ...
          <Transition name="slide-fade-sm">
            <div v-if="showProfilesMoreMenu" class="absolute right-0 mt-2 w-36 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl z-50 ring-2 ring-gray-200 dark:ring-gray-700 border border-gray-200 dark:border-gray-700">
              ...
              <button @click="$emit('delete-all-profiles'); showProfilesMoreMenu=false" class="w-full text-left px-5 py-3 text-base text-red-500 hover:text-red-600 dark:hover:text-red-400 transition-colors">清空所有</button>
            </div>
          </Transition>
        </div>
      </div>
    ...
    <Transition name="slide-fade">
      <div v-if="isBatchDeleteMode" class="mb-6 p-4 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-2xl border-2 border-purple-200 dark:border-purple-800 shadow-lg">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-2 text-sm font-semibold text-purple-700 dark:text-purple-300">
            ...
            批量删除模式
            <span v-if="selectedCount > 0" class="ml-2 px-3 py-1 bg-purple-600 text-white rounded-full text-xs font-bold shadow-md">
              已选 {{ selectedCount }}
            </span>
          </div>
    ...
    <div v-if="profiles.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
      <ProfileCard
        ...
        @edit="$emit('edit-profile', profile.id)"
        @delete="$emit('delete-profile', profile.id)"
        @change="$emit('toggle-profile', $event)"
