...
const props = defineProps({
  ...
  paginatedSubscriptions: { type: Array, required: true },
  subsCurrentPage: { type: Number, required: true },
  subsTotalPages: { type: Number, required: true },
  isSortingSubs: { type: Boolean, default: false },
...
const selectedSubIds = ref(new Set());
...
<template>
  <div class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-4 sm:p-8 lg:p-10 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <div class="relative" ref="subsMoreMenuRef">
            ...
            <Transition name="slide-fade-sm">
              <div v-if="showSubsMoreMenu" class="absolute right-0 mt-2 w-40 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl z-50 ring-2 ring-gray-200 dark:ring-gray-700 border border-gray-200 dark:border-gray-700">
                ...
                <button @click="$emit('delete-all-subs'); showSubsMoreMenu=false" class="w-full text-left px-5 py-3 text-base text-red-500 hover:text-red-600 dark:hover:text-red-400 transition-colors">清空所有</button>
              </div>
            </Transition>
          </div>
        </div>
    ...
    <Transition name="slide-fade">
      <div v-if="isBatchDeleteMode" class="mb-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-2xl border-2 border-indigo-200 dark:border-indigo-800 shadow-lg">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-2 text-sm font-semibold text-indigo-700 dark:text-indigo-300">
            ...
            批量删除模式
            <span v-if="selectedCount > 0" class="ml-2 px-3 py-1 bg-indigo-600 text-white rounded-full text-xs font-bold shadow-md">
              已选 {{ selectedCount }}
            </span>
          </div>
        ...
        <template #item="{ element: subscription }">
          <div class="cursor-move">
              <Card 
                  ...
                  @delete="$emit('delete-sub', subscription.id)" 
                  @change="$emit('toggle-sub', subscription)" 
                  @update="$emit('update-sub', subscription.id)" 
                  @edit="$emit('edit-sub', subscription.id)"
